...
<div class="block">
    ...
    <select name="WorkflowEvent_event_nxcfacebookpublish_class_{$event.id}">
...
<div class="block">
    ...
    <select name="WorkflowEvent_event_nxcfacebookpublish_class_attribute_{$event.id}">
	...
	<table class="list" cellspacing="0">
		...
		{foreach $selected_class_attributes as $class_attribute_id sequence array( 'bgdark', 'bglight' ) as $row_class}
			...
			{if $class_attribute}
				...
				<tr class="{$row_class}">
					<td><input type="checkbox" name="WorkflowEvent_event_nxcfacebookpublish_remove_class_attribute_{$event.id}[]" value="{$class_attribute.id}" /></td>
...
{* Target ID *}
<div class="block">
	<label>{'Target ID (message will be published to user`s wall if its empty)'|i18n( 'extension/nxc_social_networks' )}:</label>
	<input type="text" name="WorkflowEvent_data_nxcfacebookpublish_target_id_{$event.id}" value="{$event.data_text2|wash}" />
</div>
...
<div class="block">
	...
	<input type="checkbox" name="WorkflowEvent_data_nxcfacebookpublish_publish_only_on_create_{$event.id}" value="1" {if $event.data_int1}checked="checked"{/if} />
</div>
...
<div class="block">
	...
	<input type="checkbox" name="WorkflowEvent_data_nxcfacebookpublish_include_url_{$event.id}" value="1" {if $event.data_int2}checked="checked"{/if} />
</div>